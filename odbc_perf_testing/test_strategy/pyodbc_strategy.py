    ...
    def _connect_with_execution_details(execution_details: ExecutionDetails) -> Connection:
        ...
        if not conn_details.dsn:  # Uses custom properties if no DSN is provided for flexibility
            return connect(
                ...
                (f"trustStore={conn_details.trusted_certs};" if conn_details.trusted_certs else "") +
                (f"trustedCerts={conn_details.trust_store};" if conn_details.trust_store else "") +
                (f"trustStorePassword={conn_details.trust_store_password};"
                 if conn_details.trust_store_password else "") +
                f"useSystemTrustStore={int(conn_details.use_system_trust_store)};" +  # bool to int
                f"disableCertificateVerification={int(conn_details.disable_certificate_verification)}",  # bool to int

...
from test_cases import test_fetch_all, test_all_sql_types
...
def start_new_test_process(
    ...
    elif test_case_all_lower == 'test-all-sql-types':
        test_all_sql_types.run_all(
            process_name=test_case,
            create_executable=create_executable,
            execution_details=execution_details
        )
    elif 'test-sql-type-' in test_case_all_lower:
        type_name: str = test_case_all_lower.strip('-')[-1]  # Get type name in the end of the test case name
        test_all_sql_types.run_for_type(
            type_name=type_name,
            process_name=test_case,
            create_executable=create_executable,
            execution_details=execution_details)
...
def run(execution_details: ExecutionDetails):
    ...
    if test_details.odbc_library.lower() == 'pyodbc':
        start_new_test_process(
            ...
            lambda: PyOdbcStrategy(execution_details=execution_details),
            execution_details=execution_details
        )
    elif test_details.odbc_library.lower() == 'turbodbc':
        start_new_test_process(
            test_details.test_case,
            lambda: TurbodbcStrategy(execution_details=execution_details),
            execution_details=execution_details
        )
    else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('sql_query', help='The SQL Query to run (not applied on data type tests)')
    parser.add_argument('test_case', help='Which test case to run ["test-fetch-all"|"test-all-sql-types"]')

    # optional
    parser.add_argument('--dsn', default="", help="The ODBC Driver DSN")

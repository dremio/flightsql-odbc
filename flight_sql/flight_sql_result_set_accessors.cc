...
CreateAccessorY(FlightSqlResultSet *result_set, int column,
                const arrow::DataType &data_type, int precision, int scale,
                void *buffer, size_t buffer_length, size_t *strlen_buffer) {

#define ONE_CASE(ARROW_TYPE, ACCESSOR_CLASS)                                   \
  case ARROW_TYPE:                                                             \
    return std::unique_ptr<Accessor>(new ACCESSOR_CLASS<TARGET_TYPE>(          \
        result_set, column, TARGET_TYPE, precision, scale, buffer,             \
...
CreateAccessorX(FlightSqlResultSet *result_set, int column,
                const arrow::DataType &data_type, DataType target_type,
                int precision, int scale, void *buffer, size_t buffer_length,
                size_t *strlen_buffer) {

#define CASE_FOR_TYPE(TYPE)                                                    \
  case TYPE:                                                                   \
    return CreateAccessorY<TYPE>(result_set, column, data_type, precision,     \

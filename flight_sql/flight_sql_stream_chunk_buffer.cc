...
FlightStreamChunkBuffer::FlightStreamChunkBuffer(FlightSqlClient &flight_sql_client,
  ...
  for (const auto & endpoint : endpoints) {
    ...
    std::shared_ptr<FlightStreamChunkIterator> iterator(new FlightStreamChunkIterator(std::move(stream_reader)));
    iterators_.push_back(iterator);

    BlockingQueue<FlightStreamChunk>::Supplier supplier = [=] {
      FlightStreamChunk chunk;
      return boost::make_optional(iterator->GetNext(&chunk), chunk);
    };

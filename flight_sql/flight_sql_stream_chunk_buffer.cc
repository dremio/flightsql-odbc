...
void FlightStreamChunkBuffer::Produce(FlightStreamChunk chunk) {
    ...
    threads_.emplace_back([&] {
      ...
      while (!closed_) {
        ...
        if (iterator.GetNext(&chunk)) {
          ...
          auto duration0 = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0);
          std::cout << "time to iterator.GetNext(): " << duration0.count() << "ms." << std::endl;
          Produce(chunk);
          auto t2 = std::chrono::high_resolution_clock::now();
          auto duration1 = std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1);
          std::cout << "time to Produce(): " << duration1.count() << "ms." << std::endl;
        } else {
          break;
        }
      }
...
bool FlightStreamChunkBuffer::GetNext(FlightStreamChunk *result) {
  ...
  *result = buffer[left];
  ...
  auto t1 = std::chrono::high_resolution_clock::now();
  auto duration0 = std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0);
  std::cout << "time to consume: " << duration0.count() << "ms." << std::endl;

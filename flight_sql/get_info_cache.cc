        ...
        if (!value_array->IsNull(i)) {
          ...
          case ARROW_SQL_IDENTIFIER_CASE: {
            bool is_case_insensitive =
                static_cast<int64_t>(
                    ScalarToInt64(scalar) &
                    (1 << SqlInfoOptions::
                         SQL_CASE_SENSITIVITY_CASE_INSENSITIVE)) != 0;
            bool is_upper_case =
                static_cast<int64_t>(
                    ScalarToInt64(scalar) &
                    (1 << SqlInfoOptions::SQL_CASE_SENSITIVITY_UPPERCASE)) != 0;
            bool is_mixed_case =
                static_cast<int64_t>(
                    ScalarToInt64(scalar) &
                    (1 << SqlInfoOptions::SQL_CASE_SENSITIVITY_UNKNOWN)) != 0;
          ...
          case SqlInfoOptions::SQL_NULL_ORDERING: {
            ...
            // This is an enum but really each value should be mutually
            // exclusive. Give higher precedence to start/end options, then
            // high, then low.
            if ((scalar_value &
                 (1 << SqlInfoOptions::SQL_NULLS_SORTED_AT_START)) != 0) {
              value = SQL_NC_START;
            } else if ((scalar_value &
                        (1 << SqlInfoOptions::SQL_NULLS_SORTED_AT_END)) != 0) {
              value = SQL_NC_END;
            } else if ((scalar_value &
                        (1 << SqlInfoOptions::SQL_NULLS_SORTED_HIGH)) != 0) {
              value = SQL_NC_HIGH;
            } else {
              value = SQL_NC_LOW;
            }

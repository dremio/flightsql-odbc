...
const std::string FlightSqlConnection::STRING_COLUMN_LENGTH = "string_column_length_";
...
void FlightSqlConnection::PopulateMetadataSettings(const Connection::ConnPropertyMap &connPropertyMap) {
  metadata_settings_.string_column_length_ = GetStringColumnLength(connPropertyMap);
}

int32_t FlightSqlConnection::GetStringColumnLength(const Connection::ConnPropertyMap &connPropertyMap) {
  ...
  try {
    ...
    diagnostics_.AddWarning(
            std::string("Caught an exception while setting up " + FlightSqlConnection::STRING_COLUMN_LENGTH +

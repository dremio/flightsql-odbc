...
void FlightSqlConnection::Connect(const ConnPropertyMap &properties,
  ...
  try {
    ...
    PopulatePresentMetadataSettings(properties);
...
void FlightSqlConnection::PopulatePresentMetadataSettings(const Connection::ConnPropertyMap &connPropertyMap) {
  metadata_settings_.StringColumnLength = GetStringColumnLength(connPropertyMap);
}

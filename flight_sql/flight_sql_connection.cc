...
inline std::string GetCerts() {
  ...
  for (auto i = stores.cbegin(); i != stores.end(); i++) {
    std::shared_ptr<SystemTrustStore> cert_iterator = std::make_shared<SystemTrustStore>(*i);
...
std::shared_ptr<FlightSqlSslConfig> LoadFlightSslConfigs(const Connection::ConnPropertyMap &connPropertyMap) {
  ...
  bool use_system_trusted = AsBool(true, connPropertyMap, FlightSqlConnection::USE_SYSTEM_TRUST_STORE);

...
namespace driver
{
  namespace flight_sql
  {
    ...
    FlightSqlConnection::PopulateCallOptions(const ConnPropertyMap &props)
    {
      ...
      const boost::optional<Connection::Attribute> &connection_timeout = closed_ ? GetAttribute(LOGIN_TIMEOUT) : GetAttribute(CONNECTION_TIMEOUT);
                                       ...
                                       const std::shared_ptr<FlightSqlSslConfig> &ssl_config)
    {
      ...
      if (ssl_config->useEncryption())
      {
        ...
        catch (const std::exception &e)
        {
          /*
          diagnostics_.AddWarning(
            std::string("Error while getting AddressInfo from host. Message: ") + std::string(e.what()),
            "01000", odbcabstraction::ODBCErrorCodes_GENERAL_WARNING);
            */
        }

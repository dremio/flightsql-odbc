...
TEST(TEST_TIME32, TIME_WITH_SECONDS) {
  ...
  ArrayFromVector<Time32Type, int32_t>(value_field->type(), is_valid,
  ...
  for (int i = 0; i < t32_values.size(); ++i) {
    ...
    GetTimeForAccessor(time, t32_values[i]);
...
TEST(TEST_TIME32, TIME_WITH_MILLI) {
  ...
  std::shared_ptr<Array> time32_array;
  ArrayFromVector<Time32Type, int32_t>(value_field->type(), is_valid1,
                                       t32_values, &time32_array);
  ...
  for (int i = 0; i < t32_values.size(); ++i) {
    ...
    GetTimeForAccessor(time, convertedValue);
...
TEST(TEST_TIME64, TIME_WITH_MICRO) {
  ...
  ArrayFromVector<Time64Type, int64_t>(value_field->type(), is_valid1,

...
template <arrow::Type::type SOURCE_TYPE, DataType TARGET_TYPE>
size_t GetColumnarData(FlightSqlResultSet *result_set_, ColumnBinding *binding,
                       int64_t cells, int64_t value_offset) {
  int64_t start_offset = result_set_->GetCurrentRow();
  const std::shared_ptr<Array> &array =
      result_set_->GetArrayForColumn(binding->column)

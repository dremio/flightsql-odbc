...
RowStatus StringArrayFlightSqlAccessor<CDataType_CHAR>::MoveSingleCell_impl(
  ...
  return MoveSingleCellToCharBuffer<char>(&buffer_, binding, array, i, value_offset, update_value_offset, diagnostics);
}
...
RowStatus StringArrayFlightSqlAccessor<CDataType_WCHAR>::MoveSingleCell_impl(
  ...
  switch(GetSqlWCharSize()) {
    case sizeof(char16_t):
      return MoveSingleCellToCharBuffer<char16_t>(&buffer_, binding, array, i, value_offset, update_value_offset, diagnostics);
    case sizeof(char32_t):
      return MoveSingleCellToCharBuffer<char32_t>(&buffer_, binding, array, i, value_offset, update_value_offset, diagnostics);
    default:
      assert(false);

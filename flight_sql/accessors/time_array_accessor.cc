...
TimeArrayFlightSqlAccessor<TARGET_TYPE, ARROW_ARRAY>::TimeArrayFlightSqlAccessor(
  ARROW_ARRAY *array) : FlightSqlAccessor<ARROW_ARRAY, TARGET_TYPE, 
  TimeArrayFlightSqlAccessor<TARGET_TYPE, ARROW_ARRAY>>(array) {}

template<CDataType TARGET_TYPE, typename ARROW_ARRAY>
void TimeArrayFlightSqlAccessor<TARGET_TYPE, ARROW_ARRAY>::MoveSingleCell_impl(ColumnBinding *binding,
                                                                               ARROW_ARRAY *array, int64_t i,
                                                                               int64_t value_offset) {
  typedef unsigned char c_type;
  auto *buffer = static_cast<TIME_STRUCT *>(binding->buffer);

  auto time_type = arrow::internal::checked_pointer_cast<TimeType>(
    array->type());
  auto time_unit = time_type->unit();

  static thread_local tm time;

  auto converted_value = ConvertTimeValue<ARROW_ARRAY>(array->Value(i), time_unit);

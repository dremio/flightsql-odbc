...
uint32_t CalculateFraction(TimeUnit::type unit, uint32_t units_since_epoch) {
  // Convert the given remainder and time unit to microseconds
  // since the fraction field on TIMESTAMP_STRUCT is in microseconds.
  switch (unit) {
  case TimeUnit::SECOND:
    return 0;
  case TimeUnit::MILLI:
    ...
    return (units_since_epoch %
           ...
           1000;
    // TODO: revisit calculating the fraction for micros and nanos while

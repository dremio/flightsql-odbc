  ...
  while (fetched_rows < rows) {
    ...
    for (auto & column : columns_) {
      ...
      if (shifted_row_status_array) {
        std::fill(shifted_row_status_array, &shifted_row_status_array[rows_to_fetch], 0); // SQL_ROW_SUCCESS
      }

      size_t accessor_rows = accessor->GetColumnarData(
          &shifted_binding, shifted_row_status_array, current_row_, rows_to_fetch, value_offset, false, diagnostics_);
...
bool FlightSqlResultSet::GetData(int column_n, int16_t target_type,
  ...
  accessor->GetColumnarData(&binding, nullptr, current_row_ - 1, 1, value_offset, true, diagnostics_);

...
FlightSqlResultSet::FlightSqlResultSet(
    ...
    : metadata_(std::move(metadata)), columns_(metadata->GetColumnCount()),
      current_row_(0), num_binding_(0),
      chunk_iterator_(flight_sql_client, call_options, flight_info) {
  current_chunk_.data = nullptr;

  for (int i = 0; i < metadata_->GetColumnCount(); ++i) {
    columns_[i].reset(new FlightSqlResultSetColumn(this, i + 1));
  }
...
size_t FlightSqlResultSet::Move(size_t rows) {
  ...
  if (num_binding_ != columns_.size()) {
    for (auto &column : columns_) {
      column->get_data_offset = 0;
    }
  }
  ...
  while (fetched_rows < rows) {
    ...
    for (auto it = columns_.begin(); it != columns_.end(); *it++) {
      ...
      ColumnBinding *binding = column->binding.get();
      if (binding == nullptr)
        continue;

      size_t accessor_rows = column->GetAccessorForBinding()->GetColumnarData(
          binding, current_row_, rows_to_fetch, 0);
...
void FlightSqlResultSet::BindColumn(int column_n, CDataType target_type,
  ...
  auto &column = columns_[column_n - 1];
  if (buffer == nullptr) {
    if (column->binding != nullptr) {
      num_binding_--;
    }
    column->SetBinding(nullptr);
    return;
  }
  ...
  column->SetBinding(new ColumnBinding(column_n, target_type, precision, scale,
                                       buffer, buffer_length, strlen_buffer));
}

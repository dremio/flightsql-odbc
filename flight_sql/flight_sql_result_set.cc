...
size_t FlightSqlResultSet::Move(size_t rows) {
  ...
  while (fetched_rows < rows) {
    ...
    for (auto it = binding_.begin(); it != binding_.end(); *it++) {
      ...
      ColumnBinding *binding = *it;
      if (binding == nullptr)
        continue;

      const std::unique_ptr<Accessor> &accessor =
          GetAccessorForColumn(binding->column_, binding->target_type);

      size_t accessor_rows =
          accessor->GetColumnarData(binding, rows_to_fetch, 0);
...
bool FlightSqlResultSet::GetData(int column,
  ...
  ColumnBinding binding{.column_ = column,
  ...
  const std::unique_ptr<Accessor> &accessor =
      GetAccessorForColumn(column, target_type);
  accessor->GetColumnarData(&binding, 1, value_offset);
...
FlightSqlResultSet::GetAccessorForColumn(int column, DataType target_type) {
  const std::shared_ptr<arrow::DataType> &data_type =
      schema_->field(column - 1)->type();
  // TODO: Cache accessors
  return ::driver::flight_sql::CreateAccessor(this, *data_type, target_type);
}
...
void FlightSqlResultSet::BindColumn(int column, DataType target_type,
  ...
  if (buffer == nullptr) {
    binding_[column - 1] = nullptr;
    return;
  }
  auto *binding = new ColumnBinding{.column_ = column,
                                    .target_type = target_type,
                                    .precision_ = precision,
                                    .scale_ = scale,
                                    .buffer_ = buffer,

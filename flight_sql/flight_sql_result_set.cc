  ...
  while (fetched_rows < rows) {
    ...
    for (auto it = binding_.begin(); it != binding_.end(); *it++) {
      ...
      size_t accessor_rows =
          accessor->GetColumnarData(this, binding, current_row_, rows_to_fetch, 0);
...
bool FlightSqlResultSet::GetData(int column,
  ...
  // Note: current_row_ is always positioned at the index _after_ the one we are on
  // after calling Move(). So if we want to get data from the _last_ row fetched,
  // we need to subtract one from the current row.
...
FlightSqlResultSet::CreateAccessorForColumn(int column, DataType target_type) {

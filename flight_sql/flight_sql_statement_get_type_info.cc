...
Transform_inner(const odbcabstraction::OdbcVersion odbc_version,
  ...
  while (reader.Next()) {
    ...
    data.data_type = static_cast<int16_t>(reader.GetDataType());
    ...
    data.create_params = nullopt; // TODO: reader.GetCreateParams();
    data.nullable = reader.GetNullable();
    data.case_sensitive = reader.GetCaseSensitive();
    data.searchable = reader.GetSearchable();
    ...
    data.sql_data_type = reader.GetSqlDataType();

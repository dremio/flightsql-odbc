...
void DsnConfigurationWindow::OnCreate()
{
    ...
    int buttonPosY = max(commonGroupPosY, advancedGroupPosY);
...
int DsnConfigurationWindow::CreateAuthSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    const auto& token = config.Get(Key::AUTH_TOKEN);
    val = token.c_str();
    labels.push_back(CreateLabel(labelPosX, rowPos, LABEL_WIDTH, ROW_HEIGHT,
...
int DsnConfigurationWindow::CreateEncryptionSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    val = config.Get(Key::DISABLE_CERT_VERIFICATION).c_str();
...
void DsnConfigurationWindow::SaveParameters()
{
    ...
    if (enableEncryptionCheckBox->IsChecked())
    {
        ...
        config.Set(Key::DISABLE_CERT_VERIFICATION, disableCertVerificationCheckBox->IsChecked() ? TRUE_STR : FALSE_STR);
    }
    else
    {

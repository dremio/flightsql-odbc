...
DsnConfigurationWindow::DsnConfigurationWindow(Window* parent, config::Configuration& config) :
    CustomWindow(parent, "FlightConfigureDSN", "Configure Apache Arrow Flight SQL DSN"),
    width(480),
    height(350),
    config(config),
    accepted(false),
...
void DsnConfigurationWindow::OnCreate()
{
    ...
    int buttonPosY = max(commonGroupPosY, advancedGroupPosY) + INTERVAL;
...
int DsnConfigurationWindow::CreateConnectionSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    const char* val = config.Get(Key::dsn).c_str();
    ...
    val = config.Get(Key::server).c_str();
    labels.push_back(CreateLabel(labelPosX, rowPos, LABEL_WIDTH, ROW_HEIGHT,
        "Server:", ChildId::SERVER_LABEL));
    serverEdit = CreateEdit(editPosX, rowPos, editSizeX, ROW_HEIGHT, val, ChildId::SERVER_EDIT);

    rowPos += INTERVAL + ROW_HEIGHT;

    val = config.Get(Key::port).c_str();
    labels.push_back(CreateLabel(labelPosX, rowPos, LABEL_WIDTH, ROW_HEIGHT,
...
int DsnConfigurationWindow::CreateAuthSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    // TODO: Ensure the type is reselected on relaunching.
    authTypeComboBox->SetSelection(0);

    rowPos += INTERVAL + ROW_HEIGHT;

    const char* val = config.Get(Key::uid).c_str();
    ...
    val = config.Get(Key::auth_token).c_str();
    labels.push_back(CreateLabel(labelPosX, rowPos, LABEL_WIDTH, ROW_HEIGHT,
...
int DsnConfigurationWindow::CreateEncryptionSettingsGroup(int posX, int posY, int sizeX)
{
    ...
    const char* val = config.Get(Key::enable_encryption).c_str();
    ...
    val = config.Get(Key::encryption_cert).c_str();
    ...
    val = config.Get(Key::use_cert_store).c_str();
    ...
    useSystemCertStoreCheckBox = CreateCheckBox(editPosX, rowPos - 2, editSizeX, 2 * ROW_HEIGHT, "", 
...
void DsnConfigurationWindow::SaveParameters()
{
    ...
    config.Set(Key::dsn, text);
    serverEdit->GetText(text);
    config.Set(Key::server, text);
    ...
    if (COMMON_TAB == authSettingsGroupBox->GetSelection())
    {
        userEdit->GetText(text);
        config.Set(Key::uid, text);
        passwordEdit->GetText(text);
        config.Set(Key::pwd, text);
    }
    else
    {
        authTokenEdit->GetText(text);
        config.Set(Key::auth_token, text);
    }

    if (enableEncryptionCheckBox->IsChecked())
    {
        config.Set(Key::enable_encryption, TRUE_STR);
        certificateEdit->GetText(text);
        config.Set(Key::encryption_cert, text);
        config.Set(Key::use_cert_store, useSystemCertStoreCheckBox->IsChecked() ? TRUE_STR : FALSE_STR);
    }
    else
    {
        config.Set(Key::enable_encryption, FALSE_STR);
    }
}
...
bool DsnConfigurationWindow::OnMessage(UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        ...
        case WM_CTLCOLORSTATIC:
        {
            ...
            {
                case ChildId::AUTH_TOKEN_EDIT:
                case ChildId::NAME_LABEL:
                case ChildId::PASSWORD_LABEL:
                case ChildId::PORT_LABEL:
                case ChildId::SERVER_LABEL:
                case ChildId::USER_LABEL:
                {

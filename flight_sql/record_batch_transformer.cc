...
RecordBatchTransformer::Builder::Builder(std::shared_ptr<arrow::Schema> schema)
  ...
  auto newSchema = arrow::schema(builder_.new_fields_);

  std::vector<std::shared_ptr<arrow::Array>> arrays;

  for (const auto &item : builder_.task_collection_) {
...
std::shared_ptr<arrow::Schema> RecordBatchTransformer::GetTransformedSchema() {
  return arrow::schema(builder_.new_fields_);
}

RecordBatchTransformer::RecordBatchTransformer(
  RecordBatchTransformer::Builder builder) : builder_(std::move(builder)) {
}
} // namespace flight_sql
} // namespace driver

    ...
    if (!bearer_result.ok()) {
      ...
      if (flightStatus != nullptr) {
        if (flightStatus->code() == arrow::flight::FlightStatusCode::Unauthenticated) {
          ...
          throw CommunicationException(bearer_result.status().message() + ". Please ensure your encryption settings match the server.");
        }
      }
            ...
            if (!status.ok()) {
                ...
                if (flightStatus != nullptr) {
                    if (flightStatus->code() == arrow::flight::FlightStatusCode::Unauthenticated) {
                      ...
                      throw CommunicationException(status.message() + ". Please ensure your encryption settings match the server.");
                    }
                }

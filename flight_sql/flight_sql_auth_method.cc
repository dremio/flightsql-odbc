    ...
    if (!bearer_result.ok()) {
      ...
      if (flightStatus != nullptr) {
        if (flightStatus->code() == arrow::flight::FlightStatusCode::Unauthenticated) {
          ...
          throw CommunicationException(bearer_result.status().message() + ". Please check your connection options.");
        }
      }
    }
            ...
            if (!status.ok()) {
                ...
                if (flightStatus != nullptr) {
                    if (flightStatus->code() == arrow::flight::FlightStatusCode::Unauthenticated) {
                      ...
                      throw CommunicationException(status.message() + ". Please check your connection options.");
                    }
                }

    ...
    if (!bearer_result.ok()) {
      ...
      if (flightStatus != nullptr) {
        if (flightStatus->code() == arrow::flight::FlightStatusCode::Unauthenticated) {
          ...
          throw CommunicationException(bearer_result.status().message() + ". Please check if your encryption mode is consistency with the server options.");
        }
      }
    }
            ...
            if (!status.ok()) {
                ...
                if (flightStatus != nullptr) {
                    if (flightStatus->code() == arrow::flight::FlightStatusCode::Unauthenticated) {
                      ...
                      throw CommunicationException(status.message() + ". Please check if your encryption mode is consistency with the server options.");
                    }
                }

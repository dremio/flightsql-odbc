...
int main() {
  ...
  // Note: ResultSet is before the first row immediately after execute.

  const size_t rowsToFetch = 1;
  while(result_set->Move(1) == rowsToFetch) {
    int buffer_length = 1024;
    char result[buffer_length];
    while (result_set->GetData(1, driver::odbcabstraction::VARCHAR, 0, 0,
                               result, buffer_length, IncidntNum_length)) {

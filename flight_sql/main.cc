...
void TestBindColumn(const std::shared_ptr<Connection> &connection) {
  ...
  result_set->BindColumn(1, driver::odbcabstraction::C_CHAR, 0, 0, IncidntNum,
                         max_strlen, IncidntNum_length);
  result_set->BindColumn(2, driver::odbcabstraction::C_CHAR, 0, 0, Category,
...
void TestGetData(const std::shared_ptr<Connection> &connection) {
  ...
  while (result_set->Move(1) == 1) {
    ...
    result_set->GetData(1, driver::odbcabstraction::C_CHAR, 0, 0, result,
...
void TestBindColumnBigInt(const std::shared_ptr<Connection> &connection) {
  ...
  result_set->BindColumn(1, driver::odbcabstraction::C_SBIGINT, 0, 0,
                         IncidntNum, max_strlen, IncidntNum_length);
  result_set->BindColumn(2, driver::odbcabstraction::C_DOUBLE, 0, 0,
                         double_field, max_strlen, double_field_length);
  result_set->BindColumn(3, driver::odbcabstraction::C_CHAR, 0, 0, Category,
                         max_strlen, Category_length);

...
int main() {
  ...
  result_set->BindColumn(1, driver::odbcabstraction::VARCHAR, 0, 0, IncidntNum,max_strlen, IncidntNum_length);
  result_set->BindColumn(2, driver::odbcabstraction::VARCHAR, 0, 0, Category,max_strlen, Category_length);

  size_t fetched_rows = result_set->Move(batch_size);
  std::cout << "Fetched " << fetched_rows << " rows."<< std::endl;

  for (int i = 0; i < fetched_rows; ++i) {
    std::cout << "Row[" << i << "] IncidntNum: '" << IncidntNum[i] << "', Category: '" << Category[i] << "'" << std::endl;
  }

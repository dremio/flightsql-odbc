...
bool RegisterDsn(const Configuration& config, LPCSTR driver)
{
    ...
    if (map.find(Key::AUTH_TOKEN) != map.end()) {
        // An auth token has been defined, need to remove the UID/PWD parts.
        SQLWritePrivateProfileString(dsn.c_str(), Key::UID.c_str(), NULL, "ODBC.INI");
        SQLWritePrivateProfileString(dsn.c_str(), Key::PWD.c_str(), NULL, "ODBC.INI");
    }

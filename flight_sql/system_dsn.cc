...
void WriteDsnStringW(const char* dsn, const char* key, const char* value)
{
    SQLWritePrivateProfileStringW((LPCWSTR)dsn, (LPCWSTR)key, (LPCWSTR)value, (LPCWSTR)"ODBC.INI");
...
bool RegisterDsn(const Configuration& config, LPCWSTR driver)
{
    ...
    try
    {
        ...
        for (ArgMap::const_iterator it = map.begin(); it != map.end(); ++it)
        {
           ...
           // SQLWritePrivateProfileStringW((LPCWSTR)dsn, (LPCWSTR)key.c_str(), (LPCWSTR)value.c_str(), (LPCWSTR)"ODBC.INI");
            WriteDsnStringW(dsn, "Driver", "fgh");
        }
    ...
    catch (const std::exception& ex)
    {
        ...
        //  SQLPostInstallerError(ex.GetMessageText(), err.GetText());
    }

...
TEST(Transformer, TransformerRenameTest) {
  ...
  auto transformed_array_ptr = transformed_record_batch->GetColumnByName(
      transformed_name);

  auto original_array_ptr = original_record_batch->GetColumnByName(
    original_name);

  //Assert that the arrays are being the same and we are not creating new buffers

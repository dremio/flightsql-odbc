...
TEST(Transformer, TransformerTest) {
  ...
  const std::shared_ptr<arrow::RecordBatch> &transformedBatch = transformer.transform(
    originalRecord);

  ASSERT_NE(originalRecord->schema(), transformedBatch->schema());
  ASSERT_EQ(originalRecord->GetColumnByName(originalName), transformedBatch->GetColumnByName(transformed));
}
...
TEST(Transformer, TransformerAddEmptyVectorTest) {
  ...
  ASSERT_EQ(originalBatch->GetColumnByName(originalName), transformedBatch->GetColumnByName(transformed));
}

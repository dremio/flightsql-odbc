...
private:
  std::vector<std::unique_ptr<FlightSqlResultSetColumn>> columns_;
...
FlightSqlAccessor<ARROW_ARRAY, TARGET_TYPE>::FlightSqlAccessor(
    ARROW_ARRAY *array)
    : array_(array) {}

template <typename ARROW_ARRAY, CDataType TARGET_TYPE>
CDataType FlightSqlAccessor<ARROW_ARRAY, TARGET_TYPE>::GetTargetType() {
  return TARGET_TYPE;
}

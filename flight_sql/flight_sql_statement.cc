...
std::shared_ptr<ResultSet> FlightSqlStatement::GetTables_V3(
  ...
  if ((catalog_name && *catalog_name == "%") &&
    ...
    current_result_set_ = GetTablesV3ForSQLAllCatalogs(call_options_, sql_client_);
  } else if ((catalog_name && catalog_name->empty()) &&
             (schema_name && *schema_name == "%") &&
             (table_type && table_type->empty())) {
    current_result_set_ = GetTablesV3ForSQLAllDbSchemas(call_options_, sql_client_,
                                                        schema_name);
  } else if ((catalog_name && catalog_name->empty()) &&
             (schema_name && schema_name->empty()) &&
             (table_type && *table_type == "%")) {
    current_result_set_ = GetTablesV3ForSQLAllTableTypes(call_options_, sql_client_);
  } else {
    current_result_set_ =  GetTablesV3ForGenericUse(call_options_, sql_client_, catalog_name, schema_name,

...
/// \brief Factory for column Accessors.
...
public:
  ...
  size_t Move(size_t rows) {
    for (auto it = accessors.begin(); it != accessors.end(); *it++) {
      ...
      if (it == accessors.begin()) {
        // Stores the number of rows moved on the first column, expecting that
        // all columns move the same number of rows.
        rows = accessor_rows;

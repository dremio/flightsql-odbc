...
public:
  ...
  void AddProducer(Supplier supplier) {
    ...
    threads_.emplace_back([=] {
      while (!closed_) {
        ...
        auto item = supplier();
        if (item) {
          unique_lock.unlock();  // Unlock because Push also locks
          Push(*item);
        }
      }
  ...
  void Push(T item) {
    ...
    buffer_size_++;
    LOG_DEBUG("[BlockingQueue] Pushed an item, buffer size: {}", buffer_size_);
  ...
  bool Pop(T *result) {
    ...
    *result = std::move(buffer_[left_]);
    ...
    buffer_size_--;
    LOG_DEBUG("[BlockingQueue] Popped an item, buffer size: {}", buffer_size_);

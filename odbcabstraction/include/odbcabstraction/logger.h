...
#define __LAZY_LOG(LEVEL, ...) driver::odbcabstraction::Logger::GetInstance()->log(driver::odbcabstraction::LogLevel::LogLevel_##LEVEL, [&]() { return fmt::format(__VA_ARGS__); })
...
public:
  ...
  void log(LogLevel level, const std::function<std::string(void)> &build_message) {
    if (checkLogLevel(level)) {
      log(level, build_message());
    }
  };
};

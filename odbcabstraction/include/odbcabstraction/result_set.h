...
public:
  ...
  /// \brief Binds a column with a result buffer. The buffer will be filled with up to
  ...
  ///
  /// The parameters `buffer` and `strlen_buffer` passed to `BindColumn()` should
  /// have capacity to accommodate the rows requested, otherwise data will be
  /// truncated.
  ///
  ...
  /// If the value doesn't fit the buffer this method returns true and subsequent
  /// calls will fetch the rest of data.
  ///
